...
class Product
{
    ...
    {
        $st = Database::connection()->prepare('SELECT * FROM products WHERE id = ?');
        $st->execute([$id]);
    ...
    /**
     * Próxima SKU numérica disponível (reaproveita gaps).
     */
    public static function nextSkuForCompany(int $companyId): string
    {
        $st = Database::connection()->prepare("SELECT sku FROM products WHERE company_id = ? AND sku IS NOT NULL AND sku <> ''");
        $st->execute([$companyId]);
    ...
    /** Produto por empresa (rota pública /{empresa}/produto/{id}) */
    ...
    {
        $st = Database::connection()->prepare('DELETE FROM products WHERE id=?');
        $st->execute([$id]);
    }
  ...
  INNER JOIN products sp ON sp.id = gi.simple_product_id
        ...
        foreach ($groups as $g) {
            ...
            foreach ($rows as $row) {
                ...
                $items[] = [
                  ...
                  'product_id'        => $simpleId,      // compat com payload do admin
                  'name'              => (string)($row['name'] ?? ''),
                  'image'             => $row['image'] ?? null,
                  'base_price'        => $base,
                  'price'             => $base,          // útil para UI
                  'delta'             => $delta,
                  'delta_price'       => $delta,
    ...
    /**
     * Salva grupos (apaga e recria para simplificar).
     */
    ...
    /**
     * Produtos simples para combos (com contagem de ingredientes).
     */
    ...
    /**
     * Normaliza payload de grupos de combo vindo do admin.
     */
    ...
    /**
     * Recalcula total de um combo a partir das seleções do cliente.
     */
