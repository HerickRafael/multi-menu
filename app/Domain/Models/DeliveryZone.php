...
class DeliveryZone
{
    ...
    {
        $st = Database::connection()->prepare('UPDATE delivery_zones SET fee = GREATEST(0, fee + ?) WHERE company_id = ?');
    ...
    {
        $st = Database::connection()->prepare('DELETE FROM delivery_zones WHERE id = ? AND company_id = ?');
        $st->execute([$id, $companyId]);
    }
}
