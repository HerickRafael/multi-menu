...
// usa o helper global do seu projeto:
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-2 text-sm mt-1">
            ...
            <?php if (empty($customer) || (int)($customer['company_id'] ?? 0) !== (int)$company['id']): ?>
              <button 
                type="button"
                id="btn-open-login"
                class="px-2 py-1 rounded-lg border bg-white text-purple-900 hover:bg-slate-50 font-semibold">
                Entrar
              </button>
            <?php endif; ?>
          </div>
          ...
          <!-- Saudação + sair quando logado -->
          <?php if (!empty($customer) && (int)$customer['company_id'] === (int)$company['id']): ?>
...
<script>
  ...
  // Modal de login do cliente + submit
  (function(){
    ...
    formLogin?.addEventListener('submit', async function(ev){
      ev.preventDefault();
      msgLogin.classList.remove('hidden','text-red-600','text-green-700');
      ...
      try {
        ...
        if (data.ok) {
          msgLogin.classList.add('text-green-700');
          msgLogin.textContent = 'Login realizado!';
          setTimeout(()=>location.reload(), 600);
        } else {
          msgLogin.classList.add('text-red-600');
          msgLogin.textContent = data.message || 'Não foi possível entrar.';
        }
      } catch(e) {
        msgLogin.classList.add('text-red-600');
