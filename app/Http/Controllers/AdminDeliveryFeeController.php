...
class AdminDeliveryFeeController extends Controller
{
    ...
    private function renderPage(array $company, array $options = [])
    {
        ...
        $cities = $this->delivery->listCities($companyId, $citySearch !== '' ? $citySearch : null);
        $zones  = $this->delivery->listZones($companyId, $zoneSearch !== '' ? $zoneSearch : null);

        // Repopulação cidade (edição)
        $oldCity = $options['oldCity'] ?? [];

        if ($editCityId > 0 && !$oldCity) {
            $city = $this->delivery->findCity($editCityId, $companyId);
        ...
        if ($editZoneId > 0 && !$oldZone) {
            $zone = $this->delivery->findZone($editZoneId, $companyId);
    ...
    public function storeCity($params)
    {
        ...
        } elseif ($this->delivery->cityExists($companyId, $name)) {
    ...
    public function updateCity($params)
    {
        ...
        if ($id <= 0 || !$this->delivery->findCity($id, $companyId)) {
        ...
        if ($name === '') {
            $errors[] = 'Informe o nome da cidade.';
        } elseif ($this->delivery->cityExists($companyId, $name, $id)) {
            $errors[] = 'Esta cidade já está cadastrada.';
        }
        ...
        $this->delivery->updateCity($id, $companyId, $name);
    ...
    public function destroyCity($params)
    {
        ...
        if ($id > 0) {
            $this->delivery->deleteCity($id, (int)$company['id']);
        }
    ...
    public function storeZone($params)
    {
        ...
        // cidade
        $city = $cityId > 0 ? $this->delivery->findCity($cityId, $companyId) : null;
        ...
        // duplicidade
        if (!$errors && $this->delivery->zoneExists($companyId, $cityId, $neighborhood)) {
    ...
    public function updateZone($params)
    {
        ...
        $zone = $id > 0 ? $this->delivery->findZone($id, $companyId) : null;
        ...
        if (!$errors && $this->delivery->zoneExists($companyId, $cityId, $neighborhood, $id)) {
            $errors[] = 'Este bairro já está cadastrado para a cidade selecionada.';
        }
        ...
        $this->delivery->updateZone($id, $companyId, [
          'city_id'      => $cityId,
          'neighborhood' => $neighborhood,
          'fee'          => $feeValue,
        ]);
    ...
    public function adjustZones($params)
    {
        ...
        $this->delivery->adjustFees($companyId, $delta);
    ...
    public function destroyZone($params)
    {
        ...
        if ($id > 0) {
            $this->delivery->deleteZone($id, (int)$company['id']);
        }
    ...
    public function updateOptions($params)
    {
        ...
        $this->companies->updateDeliveryOptions($companyId, (float)$normalized, $freeFlag);
