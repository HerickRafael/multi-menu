...
class CustomerAuthController extends Controller
{
    ...
    {
        return $this->customers->findCompanyBySlug($slug);
    }
    ...
    public function login(array $params): void
    {
        ...
        $customer = $this->customers->findByCompanyAndWhatsapp((int)$company['id'], $e164);
        ...
        if (!$customer) {
            ...
            $customer = $this->customers->findById((int)$id);
        } else {
            // atualiza
            $this->customers->updateById((int)$customer['id'], [
                'name'          => $name,
                'whatsapp'      => $whatsRaw,
                'updated_at'    => $now,
                'last_login_at' => $now,
            ]);
            $customer = $this->customers->findById((int)$customer['id']);
        }
