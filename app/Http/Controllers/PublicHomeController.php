...
class PublicHomeController extends Controller
{
    ...
    {
        $st = $this->db()->prepare('SELECT * FROM company_hours WHERE company_id=? ORDER BY weekday');
    ...
    public function index($params)
    {
        ...
        $categories   = $this->categories->listActiveByCompany($cid);
        // Cardápio completo
        $products     = $this->products->listByCompany($cid);
        // Resultados da busca (se houver termo)
        $searchResults = ($q !== '') ? $this->products->listByCompany($cid, $q) : [];
        ...
        $novidades    = $this->products->listNovelties($cid, $diasNovidade, 12);
        $maisPedidos  = $this->products->listBestSellers($cid, 12);
    ...
    public function buscar($params)
    {
        ...
        if ($isAjax) {
            ...
            $products = $this->products->listByCompany($cid, $q);
