...
class AdminSettingsController extends Controller
{
    ...
    private function loadHours(int $companyId): array
    {
        ...
        if (!$rows) {
            for ($d = 1;$d <= 7;$d++) {
                $this->db()->prepare('INSERT INTO company_hours (company_id, weekday, is_open) VALUES (?,?,0)')
                   ->execute([$companyId,$d]);
            }
            $st->execute([$companyId]);
            $rows = $st->fetchAll();
    ...
    public function save($params)
    {
        ...
        for ($d = 1;$d <= 7;$d++) {
            ...
            $this->db()->prepare('UPDATE company_hours SET is_open=?, open1=?, close1=?, open2=?, close2=? WHERE company_id=? AND weekday=?')
