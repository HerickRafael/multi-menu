...
env:
	...
	if [ -n "$(PHP_BIN)" ]; then \
		...
		$(DOCKER_COMPOSE_SCRIPT) exec --rm --no-deps app sh -lc '\
...
composer-install:
	@if [ -f composer.json ]; then \
		if [ -n "$(COMPOSER_BIN)" ]; then \
			...
			$(DOCKER_COMPOSE_SCRIPT) exec --rm --no-deps app composer install --no-interaction --prefer-dist; \
		else \
...
npm-install:
	@if [ -f package.json ]; then \
		if [ -n "$(NPM_BIN)" ]; then \
			...
			$(DOCKER_COMPOSE_SCRIPT) exec --rm --no-deps node npm install; \
		else \
...
migrate:
	@$(EXECUTAR_TAREFA_APP) migrate

seed:
	@$(EXECUTAR_TAREFA_APP) seed
